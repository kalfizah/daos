...
daos_size_t	ts_nvme_size = 0;		/* default pool NVMe size */
...
"	$ daos_perf -s 1024k -A -R 'U U;o=4k;s=4k V'\n\n"
...
{
	int		c_opt_len;
	int		c_optstr_len = strlen(perf_common_optstr);
	int		optstr_len = strlen(optstr_in);
	struct option	*opts;
	char		*optstr;
	int		i, j;

	c_opt_len = sizeof(perf_common_opts)/sizeof(perf_common_opts[0]);
	opt_len += c_opt_len;
